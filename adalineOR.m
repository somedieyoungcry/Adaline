...
yi = -b/w(2);
disp('Inicio:');
fprintf('Ganancia: %f\n',b); #Imprime la ganancia inicial
fprintf('Peso: [%f %f]\n',w); # Imprime el peso inicial
fprintf('X(0)= %f ,Y(0)=%f\n',xi,yi);
plot(xi,yi,'xblack');
...
for n = 1: 100
    fprintf('Iteracion %d\n',n); #Indica el numero de iteracion en la que estamos
    cont = 0;
    for i = 1 : size(P,2)
    ...
    fprintf('Ganancia: %f\n',b); #Imprime la ganancia en la ultima iteracion
    fprintf('Peso: [%f %f]\n', w); #Imprime el peso en la ultima iteracion 
    xi = -b/w(1);
    yi = -b/w(2);
    fprintf('X(%d)= %f ,Y(%d)=%f\n',n,xi,n,yi);
    plot(xi,yi,'xblack');
    if(cont==size(P,2))
        x = -10:0.1:10;
        y = -w(1)*x/w(2) - b/w(2);
        plot(x,y,'black'); #Grafica la ganancia y el peso de las ultimas iteraciones
        break;
    end
   disp('**********************************************'); 
end
